...
def COCO2017(path: str) -> Dataset:
    ...
    for segment_name in ("test", "unlabeled"):
        segment = dataset.create_segment(segment_name)
        for image_path in glob(os.path.join(root_path, f"{segment_name}2017", "*.jpg")):
...
def _save_mask(
    ...
    array = np.array(
        Image.open(
            os.path.join(annotation_path, f"panoptic_{segment_name}2017", f"{image_stem}.png")
        ),
        dtype=int,
    )
    ...
    new_mask_path = os.path.join(new_file_path, f"{image_stem}.png")
